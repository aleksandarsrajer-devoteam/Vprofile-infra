locals {
  image_name   = "vprofile-tomcat-${formatdate("YYYYMMDDHHmmss", timestamp())}-${substr(var.git_sha, 0, 8)}"
  image_family = "vprofile-tomcat"
}
...
source "googlecompute" "vprofile_tomcat" {
  ...
  image_description       = "VProfile Tomcat — built from git commit ${var.git_sha}"
